...
class ClientGUI:
    ...
    def _create_widgets(self):
        ...
        tk.Label(top_frame, text=f"☁  {APP_NAME} - 客户端", font=FONT_TITLE, fg=theme["topbar_text"], bg=theme["topbar_bg"]).pack(side=tk.LEFT, padx=20, pady=10)
        self.status_label = tk.Label(top_frame, text="● 未连接", font=FONT_NORMAL, fg="#f56c6c", bg=theme["topbar_bg"])
    ...
    def _create_sidebar_menu(self):
        ...
        self.sidebar_buttons = {}
        self.sidebar_frames = {}
        menu_items = [("print", "快速打印"), ("virtual", "虚拟打印"), ("tasks", "我的任务"), ("settings", "设置")]
    ...
    def _switch_page(self, page_key):
        ...
        for key in self.sidebar_frames:
            ...
            if key == page_key:
                bg = theme["sidebar_active"]; fg = "#ffffff"
            else:
